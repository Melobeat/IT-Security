...
def run():
    ...
    a = hex_string_to_int(sys.argv[1])
    b = hex_string_to_int(sys.argv[2])
    ...
    p = hex_string_to_int(p_hex)

    A = pow(g, a, p)
    B = pow(g, b, p)
    print(int_to_hex_string(A))
    print(int_to_hex_string(B))

    K = pow(B, a, p)
    print(int_to_hex_string(K))

    H = hashlib.sha512(int_to_hex_string(K).encode('utf-8')).hexdigest()
    print(H)
